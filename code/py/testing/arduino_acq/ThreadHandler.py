...
# Class: ThreadHander
# Modifications: Italo G S Fernandes and Paulo Camargos
...
class ThreadHandler:
    ...
    args : tuple
        Tuple of arguments to the worker function
    ...
    ...
    """
    def __init__(self, worker=None,args=None,on_end_function=None):
        self.thread = Thread(target=self.run, args=None)
        self.args = args
        self.worker = worker
    ...
    def start(self):
        ...
        if not self.isAlive:
            argumento = self.args
            self.thread = Thread(target=self.run, args=(argumento,))
            print(f'arg {self.args}')
            self.isAlive = True
            self.isRunning = True
            self.thread.start()
            self.isFinished = False
